...
class MedicineAdmin(ImportExportActionModelAdmin):
    list_display = ("id", "medicine_name", 'code_name', 'medicine_type_id',
                    'add_charge', 'medicine_details', 'created_at', 'update_at')
...
class medicineBatchesAdmin(admin.ModelAdmin):
    list_display = ("id", 'medicine_id', 'barcode', 'medicine_strg_id', "batch_no", "status",
                    "created_at", 'update_at')
...
class EmployeeAdmin(admin.ModelAdmin):
    list_display = ("id", "user", "name", "employee_type", "phone_no",
                    "gender", "qualification", "dob", "address", "email_address")


class patientBillRecordsAdmin(admin.ModelAdmin):
    list_display = ("id", "patient", "desp", "pres", "medicine", "boxes_stored",
                    "strips_stored", "pieces_stored", 'amount', "datevisited")


class patPrescriptionBillRecordHistoryAdmin(admin.ModelAdmin):
    list_display = ("id", "pres", "total", "paid_amount",
                    "amount_given", "change", "status")


class presBillSummaryAdmin(admin.ModelAdmin):
    list_display = ("id", "pres", "net_total", "paid_amount",
                    "amount_given", "change", "status")


class patientMedRecordsAdmin(admin.ModelAdmin):
    list_display = ("id", "patient", "emp_doc", "pres",
                    "medicine", "prescription", "datevisited")


class medInfoRecordAdmin(admin.ModelAdmin):
    list_display = ("id", "patient", "medicine",
                    "pres", "timing", "datevisited",)
...
class procedureBillRecordAdmin(admin.ModelAdmin):
    list_display = ('id', 'procedure', 'pres',
                    'net_total', 'status', 'date_visited')
...
class RoomsAdmin(admin.ModelAdmin):
    list_display = ('id', 'floor', 'room_no', 'charge_per_day',
                    'ac_charge_per_day', 'status')
...
class revisitHistoryAdmin(admin.ModelAdmin):
    list_display = ("id", "patient", "patient_type", "doc", "pres", "sign_symtoms", "provisional_diagnosis",
                    "investigation", "diagnosis", "vitals", "rx", "admit_reason", "date_visited")
...
class procedureBillSummaryAdmin(admin.ModelAdmin):
    list_display = ('id', 'procbr', 'pres', "net_total")
...
class rawFilesTableAdmin(admin.ModelAdmin):
    list_display = ("id", "file_name", "file", "size",
                    "month", "year", "uploaded_by")


class attendanceRecordsAdmin(admin.ModelAdmin):
    list_display = ("id", "emp_name", "emp_user", "date", "checkin", "checkout",
                    "status", "day", "month", "year", "hours_worked", "minutes_worked", "monthyear")
...
admin.site.register(tt_Medicine_DespensoryStock,
                    tt_Medicine_DespensoryStockAdmin)
...
admin.site.register(patPrescriptionBillRecordHistory,
                    patPrescriptionBillRecordHistoryAdmin)
