...
function mainDashBoard() {
  ...
  var main_page_content = $("#main_page_content").append(
    '<div class="container-fluid" id="container-med-dashboard"></div>'
  );
...
function retrieveMedicineNames() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      medicine_name_type_list = JSON.parse(
        JSON.stringify(data["medicine_name_type_list"])
      );
      $(".modal-loading").hide();
    },
  });
...
function retrieveMedicineGenDataFromStock() {
  ...
  $.ajax({
    ...
    success: function (data) {
      medicine_batch_in_stock_dict = JSON.parse(
        data["medicine_batch_in_stock_dict"]
      );
...
function addMedicineForm() {
  ...
  $("#container-med-dashboard").append(
    "<h2 class='center_h_tag_forms'>Add Medicine To Inventory</h2>"
  );
  $("#container-med-dashboard").append("<hr>");
  ...
  var row_code_name = $("<div class='row' id='row_code_name'></div>");
  ...
  col_one__row_div_one__col_one__row_div_one = $(
    "<div class='col-md-4' style=' padding-top:10px'><label class='custom_label_css'>Medicine</label></div>"
  );

  col_two__row_div_one__col_one__row_div_one = $(
    "<div class='col-md-8' style=' padding-top:10px' onfocusout='focusOut_medicineNameAddMedForm($(this))'id='med_name_input_div'><input type='text' id='med_name_input' class='form-control-custom'></input></div>"
  );

  $(row_div_one__col_one__row_div_one).append(
    col_one__row_div_one__col_one__row_div_one
  );
  $(row_div_one__col_one__row_div_one).append(
    col_two__row_div_one__col_one__row_div_one
  ...
  col_one__row_div_one__col_two__row_div_one = $(
    "<div class='col-md-4' style=' padding-top:10px'><label class='custom_label_css float-right'>Medicine Type</label></div>"
  );

  var select = $(
    "<select id='med_type_sel' onchange='checkMedTypeReg($(this))' class='form-control-custom'></select>"
  );
  var option = $(
    "<option selected='selected' id=" +
      medicine_type_list[0] +
      "-opt value=" +
      medicine_type_list[0] +
      ">" +
      medicine_type_list[0] +
  ...
  col_two__row_div_one__col_two__row_div_one = $(
    "<div class='col-md-8 '></div>"
  );
  $(col_two__row_div_one__col_two__row_div_one).append(select);
  $(row_div_one__col_two__row_div_one).append(
    col_one__row_div_one__col_two__row_div_one
  );
  $(row_div_one__col_two__row_div_one).append(
    col_two__row_div_one__col_two__row_div_one
  ...
  // HTML of row2 starting.

  code_name1 = $(
    "<div class='col-md-4' style=' padding-top:10px'><label class='custom_label_css'>Medicine Code Name</label></div>"
  );

  code_name2 = $(
    "<div class='col-md-4' style=' padding-top:10px' id='med_name_input_div'><input type='text' value=''  id='med_code_name_input' class='form-control-custom'></input></div>"
  );

  $(row_code_name).append(code_name1);
  $(row_code_name).append(code_name2);

  // Html of row 3 starting.

  col_one__package_info_row_div = $(
    "<div class='col-md-2' style=' padding-top:10px'></div>"
  );
  p__col_one__package_info_row_div = $(
    "<label class='custom_label_css'>Medicine Details</label>"
  );
  col_one__package_info_row_div.append(p__col_one__package_info_row_div);
  col_two__package_info_row_div = $(
    "<div class='col-md-8' style=' padding-top:10px'></div>"
  );
  p__col_two__package_info_row_div = $(
    "<input type='textarea' class='form-control-custom' id='med_details'></input>"
  );
  ...
  col_one__row_div_three = $(
    "<div class='col-md-2' style=' padding-top:10px'></div>"
  );
  p__col_one__row_div_three = $(
    "<label class='custom_label_css'>Add Charge Status</label>"
  );
  col_one__row_div_three.append(p__col_one__row_div_three);
  col_two__row_div_three = $(
    "<div class='col-md-3' style=' padding-top:10px'></div>"
  );
  p__col_two__row_div_three1 = $(
    "<input type='radio' name='YES_NO' value='YES' >YES</input>"
  );
  p__col_two__row_div_three2 = $(
  ...
  col_one__weight_div_row = $(
    "<div  class='col-md-2' style=' padding-top:10px'></div>"
  );
  p__col_one__weight_div_row = $(
    "<label class='custom_label_css'>Weight (mg)</label>"
  );
  col_one__weight_div_row.append(p__col_one__weight_div_row);

  col_two__weight_div_row = $(
    "<div class='col-md-3' style=' padding-top:10px'></div>"
  );
  p__col_two__weight_div_row = $(
    "<input class='form-control-custom' type='number' min=0 id='med_weight_input'></input>"
  );
  ...
  p__col_two = $(
    "<input class='form-control-custom' type='number' min=0 id='med_litre_input'></input>"
  );
  ...
  save_btn_label = $(
    "<button class='save_btn fa fa-save' style='width:inherit' onclick='saveMedicineToDb()'>  Save</button>"
  );
  ...
  $(main_col_div).append(row_code_name);
...
function saveMedicineToDb() {
  ...
  if ($("#med_name_input").val() === "") {
    var div = $(
      "<div id='empty_name_check_div'><span  class='glyphicon' style='color:red'>&#x2a;Required</span></div>"
    );
  ...
  for (lst in medicine_name_type_list) {
    ...
    if (
      JSON.stringify(med_lst) == JSON.stringify(medicine_name_type_list[lst])
    ) {
      ...
      $("#dialog-confirm").dialog({
        ...
        buttons: {
          Save: function () {
            ...
            var code_name = $("#med_code_name_input").val();
...
function sendAjaxReqToSaveMedicineToDb(
  medicine_name,
  selected_type,
  med_details,
  add_charge_status,
...
function medicineDataTableGenerator(list) {
  ...
  $(function () {
    $("#med_table").append(
      '<caption style="caption-side: top;text-align: center;" class="datatable_heading_label">Registered Medicines</caption>'
    );
    med_table_datatable = $("#med_table").DataTable({
      data: list,
      columns: [{ title: "Medicine" }, { title: "Type" }],
      paging: false,
      scrollY: 200,
      scrollX: true,
      ordering: false,
...
function addMedicineToWhStockFrom() {
  ...
  $("#main_page_content").empty();
  $("#main_page_content").append(
    '<div class="container-fluid" id="container-med-dashboard"></div>'
  );
  $("#container-med-dashboard").append(
    "<h2 class='center_h_tag_forms'>Add Medicine To Storage</h2>"
  );
  $("#container-med-dashboard").append("<hr class='custom_hr'>");
  var main_row_div = $("<div class='row removerowmargins_div'></div>");
  $("#container-med-dashboard").append(main_row_div);
  var main_col_div = $(
    "<div class='col-md-8 genformdiv1 ' id='medstorage_main_col_div'></div>"
  );
  ...
  var row_div_one = $(
    "<div class='row' style='padding-bottom:10px;padding-top:10px'></div>"
  );
  // Medicine Name
  ...
  med_name_label = $(
    "<label for='medicine_name_tag' class='custom_label_css'>Medicine Name</label>"
  );
  med_name_input = $(
    "<input class='form-control-custom custom_input_css' id='medicine_name_tag' onfocusout='focusOut_medicineName($(this))'>"
  );
  ...
  med_type_label = $(
    "<label class='custom_label_css float-right'>Medicine Type</label>"
  );
  colmd1.append(med_type_label);
  med_type_input = $(
    "<input class='form-control-custom custom_input_css' id='medicine_type_input' ></input>"
  );
  ...
  colmd2 = $("<div class='col-md-8'></div>");
  batchno_label = $(
    "<label class='custom_label_css float-right'>Batch No</label>"
  );
  colmd1.append(batchno_label);
  batchno_input = $(
    "<input class='form-control-custom custom_input_css' id='batchno_input' onfocus='checkBatchNo()'></input>"
  );
  ...
  var table = $(
    '<table id="med_table" class="datatablecss_med "  style="width: 100%;"></table>'
  );
  divrow.append(table);
  ...
  var divrow = $("<div class='genformdiv2'></div>");
  var table = $(
    '<table id="med_in_stock" class="datatablecss_med"  style="width: 100%;" ></table>'
  );
  divrow.append(table);
  $(main_col_div1).append(divrow);
...
function createMedTStckT() {
  $(function () {
    $("#med_table").append(
      '<caption style="caption-side: top;text-align: center;" class="datatable_heading_label">Regestered Medicines</caption>'
    );
    med_table_datatable = $("#med_table").DataTable({
    ...
    $("#med_in_stock").append(
      '<caption style="caption-side: top;text-align: center;" class="datatable_heading_label">Medicines in Stock</caption>'
    );
    medstck_datatable = $("#med_in_stock").DataTable({
      data: medicine_batch_in_stock_list,
      columns: [{ title: "Medicine" }, { title: "BatchNo" }],
      paging: false,
      scrollY: 200,
      scrollX: true,
      ordering: true,
...
function subpackage_typeOnSelect(element) {
  ...
  console.log(
    "parent id ",
    $(element).parent().parent().parent().parent().attr("id")
  );
  ...
  if (index === sublevel_list.length - 1) {
    ...
    if (pack_type !== "Piece" && pack_type !== "--") {
      var temp_subpack_type_list = JSON.parse(
        JSON.stringify(subpack_type_list)
      );
    ...
    if (index > -1) {
      // subpack_type_list.shift(index);
      deleted_sublevel_list = sublevel_list.splice(
        index + 1,
        sublevel_list.length
      );
...
function subPackageHtml(pack_type, sublevel, subpack_type_list) {
  ...
  var row_div = $(
    "<div class='row subpacks' id='subpack_row_div_sublevel" +
      sublevel +
      "' ></div>"
  );
  var col_one__row_div = $(
    "<div class='col-md-4'  id='subpack_info_col_div_sublevel" +
      sublevel +
      "'></div>"
  );

  var row_one__col_one__row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  colmd1 = $("<div class='col-md-4'></div>");
  colmd2 = $("<div class='col-md-8'></div>");

  var subpackage_type_label = $(
    "<label  class='sub_custom_label_css'>Sub Package Type</label>"
  );
  colmd1.append(subpackage_type_label);

  var subpackage_type_select = $(
    "<select onchange='subpackage_typeOnSelect($(this))' id='subpackage_type_select" +
      pack_type +
  ...
  // new version
  row_two__col_one__row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  var colmd1 = $("<div class='col-md-4'></div>");
  var colmd2 = $("<div class='col-md-8'></div>");

  quantity_subpackage_type_label = $(
    "<label  class='sub_custom_label_css' >Quantity:</label>"
  );
  quantity_subpackage_type_input = $(
    "<input class='form-control-custom' type='number'  step='1.00' min='1' id='sublevel" +
      sublevel +
      "' name='sublevel_input'></input>"
  );
  ...
  var col_two__row_div = $(
    "<div ><button id='delBtn' class='fas fa-2x   fa-times-circle' onclick='deleteSubPackdiv($(this))'></button></div>"
  );
...
function calculateFunc() {
  ...
  sublevel_data = $("[id^=subpack_row_div_sublevel]")
    .map(function () {
  ...
  if (sublevel_data.length !== 1) {
    ...
    var container = $(
      '<div class="container-left" id="container_sublevels"> </div>'
    );
    var row = $("<div class='row' id='row_sublevels'></div>");
    var col = $("<div class='col-md-12' id='row_sublevels_cols'></div>");

    var row_div_zero = $(
      "<div class='row' id='sublevelsmainrow_" + selectedoption + "'></div>"
    );
    ...
    priceofonepack_label = $(
      "<label id='" +
        selectedoption +
        "_unit-label' >Price of one " +
        selectedoption +
        ":</label>"
    );
    priceofonepack_input = $(
      "<span id='" +
        selectedoption +
        "_unit-price' class='form-control-custom-static'>" +
        priceperpack +
        "</span>"
    );
    ...
    var row_div_two = $(
      "<div class='row' id='sublevelssubrowquantity_Piece'></div>"
    );
    var col_one__row_div = $("<div class='col-md-12'></div>");
    var row1_col_one__row_div = $("<div class='row'></div>");
    var colmd1 = $("<div class='col-md-9'></div>");
    var colmd2 = $("<div class='col-md-2'></div>");
    totalnoofpieces_label = $(
      "<label id='Piece_total-label' value='Piece'>Total no of Pieces:</label>"
    );
    totalnoofpieces_input = $(
      "<span id='Piece_total-quant' class= 'form-control-custom-static'>" +
        totalnoofpieces +
        "</span>"
    );
    ...
    var row_div_three = $(
      "<div class='row'  id='sublevelssubrowprice_Piece'></div>"
    );
    var col_one__row_div = $("<div class='col-md-12'></div>");
    var row2_col_one__row_div = $("<div class='row'></div>");
    var colmd1 = $("<div class='col-md-9'></div>");
    var colmd2 = $("<div class='col-md-2'></div>");
    var prieceofonepiece_label = $(
      "<label id='Piece_unit-label ' value='Piece' >Price of Piece:</label>"
    );
    var prieceofonepiece_input = $(
      "<span id='Piece_unit-price' class='form-control-custom-static'>" +
        priceof1piece +
        " </span>"
    );
...
function recursiveFunc(list_length) {
  ...
  if (list_length > 0) {
    ...
    if (recursive_count == 0) {
      ...
      var row_div_one = $(
        "<div class='row' id='sublevelssubrowquantity_" +
          package_name +
          "'></div>"
      );
      var col_one__row_div = $("<div class='col-md-12'></div>");
      ...
      var colmd1 = $("<div class='col-md-9'></div>");
      var colmd2 = $("<div class='col-md-2'></div>");
      var totalofsublevel_label = $(
        "<label id='" +
          package_name +
          "_total-label' >Total no of " +
          package_name +
          ":</label>"
      );
      var totalofsublevel_input = $(
        "<span id='" +
          package_name +
          "_total-quant' class='form-control-custom-static'>" +
          totalofsublevel1 +
          " </span>"
      );
      ...
      var row_div_two = $(
        "<div class='row' id='sublevelssubrowprice_" + package_name + "'></div>"
      );
      var col_one__row_div = $("<div class='col-md-12'></div>");
      var row2_col_one__row_div = $("<div class='row'></div>");
      var colmd1 = $("<div class='col-md-9'></div>");
      var colmd2 = $("<div class='col-md-2'></div>");
      var priceofonepack_label = $(
        "<label id='" +
          package_name +
          "_unit-label'>Price of one " +
          package_name +
      ...
      package_name =
        previous_sublevel_data[previous_sublevel_data.length - 1][1];
      quantityofsublevel =
        previous_sublevel_data[previous_sublevel_data.length - 1][2];
      priceof1sublevel =
        previous_sublevel_data[previous_sublevel_data.length - 1][3];
      ...
      var priceofonepack_input = $(
        "<span  id='" +
          package_name1 +
          "_total-price' class='form-control-custom-static'>" +
          totalnoofsublevel +
          "</span>"
      );
      colmd1.append(priceofonepack_label);
      ...
      var row_div_two = $(
        "<div class='row' id='sublevelssubrowprice_" +
          package_name1 +
          "'></div>"
      );
      var col_one__row_div = $("<div class='col-md-12'></div>");
      ...
      $("#dialog-confirm").dialog({
        ...
        buttons: {
          Save: function () {
            ...
            packages_priceandquant_dict[main_package_type] = [
              parseInt(main_quantity_input),
            ];
            sublev = [];
            $("[id^=subpack_row_div_sublevel]")
              .map(function () {
                var selectedoption = $(this).find("select").val();
                var inputval = $(this).find("input").val();
                // var inputid=$(this).find('input').attr('id')
            ...
            $("#row_sublevels")
              ...
              .map(function () {
                ...
                if (packages_priceandquant_dict[package_name] !== undefined) {
                  packages_priceandquant_dict[package_name].push(
                    parseInt(main_package_unit)
                  );
                }
              })
              .get();
            ...
            $("#row_sublevels")
              ...
              .map(function () {
                ...
                if (packages_priceandquant_dict[package_name] === undefined) {
                  packages_priceandquant_dict[package_name] = [
                    package_name_unit_or_total,
                  ];
                } else {
                  packages_priceandquant_dict[package_name].push(
            ...
            $.ajax({
              ...
              data: {
                ...
                packages_priceandquant_dict: JSON.stringify(
                  packages_priceandquant_dict
                ),
              },
...
function addRowDivTwo() {
  ...
  var container_row_div_two = $(
    "<div class='col-md-12' id='cont_row_div_two'></div>"
  );

  var package_info_row_div = $(
    "<div class='row' id='package_info_row_div'></div>"
  );
  col_one__package_info_row_div = $(
    "<div class='col-md-7'  id='package_info_col_div'></div>"
  );

  row_one__col_one__package_info_row_div = $("<div class='row'></div>");
  var col_one__row_one__col_one__package_info_row_div = $(
    "<div class='col-md-12'></div>"
  );
  row__col_one__row_one__col_one__package_info_row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  colmd1 = $("<div class='col-md-4'></div>");
  colmd2 = $("<div class='col-md-8'></div>");

  mainpackage_type_label = $(
    "<label class='sub_custom_label_css'>Main Package Type</label>"
  );
  colmd1.append(mainpackage_type_label);

  var mainpackage_type_select = $(
    "<select class='form-control-custom' onchange='package_typeOnSelect($(this))' id='mainpackage_type_select'></select>"
  );
  var option = $("<option value='--'>--</option>");
  ...
  col_one__row_one__col_one__package_info_row_div.append(
    row__col_one__row_one__col_one__package_info_row_div
  );

  row_one__col_one__package_info_row_div.append(
    col_one__row_one__col_one__package_info_row_div
  );

  row_two__col_one__package_info_row_div = $("<div class='row'></div>");
  var col_one__row_two__col_one__package_info_row_div = $(
    "<div class='col-md-12'></div>"
  );
  row__col_one__row_two__col_one__package_info_row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  colmd1 = $("<div class='col-md-4'></div>");
  colmd2 = $("<div class='col-md-8'></div>");

  quantity_mainpackage_type_label = $(
    "<label class='sub_custom_label_css'>Quantity</label>"
  );
  colmd1.append(quantity_mainpackage_type_label);
  quantity_mainpackage_type_input = $(
    "<input class='form-control-custom' type='number'  step='1.00' min='1'  id='main_quantity_input' name='main_quantity_input'></input>"
  );
  ...
  col_one__row_two__col_one__package_info_row_div.append(
    row__col_one__row_two__col_one__package_info_row_div
  );
  row_two__col_one__package_info_row_div.append(
    col_one__row_two__col_one__package_info_row_div
  );

  row_three__col_one__package_info_row_div = $("<div class='row'></div>");
  var col_one__row_three__col_one__package_info_row_div = $(
    "<div class='col-md-12'></div>"
  );
  row__row_three__col_one__package_info_row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  colmd1 = $("<div class='col-md-4'></div>");
  colmd2 = $("<div class='col-md-8'></div>");

  priceperpackage_label = $(
    "<label class='sub_custom_label_css' >Purchase Rate</label>"
  );
  colmd1.append(priceperpackage_label);
  priceperpackage_input = $(
    "<input class='form-control-custom' type='number'  step='50.00' min='0'  id='main_priceperpack_input' name='main_priceperpack_input'></input>"
  );
  ...
  col_one__row_three__col_one__package_info_row_div.append(
    row__row_three__col_one__package_info_row_div
  );
  row_three__col_one__package_info_row_div.append(
    col_one__row_three__col_one__package_info_row_div
  );
  ...
  col_one__package_info_row_div.append(
    row_three__col_one__package_info_row_div
  );

  col_two__package_info_row_div = $(
    "<div class='col-md-4' id='calc_res_info_form'></div>"
  );
...
function addRowDivThree() {
  ...
  manufact_date_label = $(
    "<label class='sub_custom_label_css' for='from'>Manufactoring Date</label>"
  );
  manufact_input = $(
    "<input class='form-control-custom'  autocomplete='off' id='manudatepicker'>"
  );
  ...
  exp_date_label = $(
    "<label class='sub_custom_label_css' for='to'>Expiration  Date</label>"
  );
  exp_date_input = $(
    "<input class='form-control-custom'  autocomplete='off' id='expdatepicker'>"
  );
...
function addRowDivFour() {
  ...
  var calculate_button = $(
    '<button class="calcbtn fa fa-calculator" id="calculate_button_div" onclick="calculateFunc()">  Calculate</button>'
  );
  colmd1.append(calculate_button);

  savetomedstrgForm_button = $(
    '<button class="save_btn_normal fa fa-save" onclick="saveMedicineToWhStock()" style="width:inherit">  Save</button>'
  );
...
function addRowDivTwoCustomSyrup() {
  ...
  var container_row_div_two = $(
    "<div class='col-md-12 container-left' id='cont_row_div_two'></div>"
  );
  var package_info_row_div = $(
    "<div class='row' id='package_info_row_div'></div>"
  );
  var col_one__package_info_row_div = $(
    "<div class='col-md-7'  id='package_info_col_div'></div>"
  );

  var row_one__col_one__package_info_row_div = $(
    "<div class='row' style='padding-bottom:10px;padding-top: 23px;'></div>"
  );
  var colmd1 = $("<div class='col-md-4'></div>");
  var colmd2 = $("<div class='col-md-8'></div>");
  mainpackage_type_label = $(
  ...
  var mainpackage_type_select = $(
    "<select class='form-control-custom' onchange='customPackage_typeOnSelect($(this))' id='mainpackage_type_select'></select>"
  );
  var option = $("<option value='--'>--</option>");
  $(mainpackage_type_select).append(option);
  // if medicine type is syrup then populate with this data
  ...
  row_two__col_one__package_info_row_div = $(
    "<div class='row main_inputfields_bottle' style='padding-bottom:10px' ></div>"
  );
  var colmd1 = $("<div class='col-md-4'></div>");
  var colmd2 = $("<div class='col-md-8'></div>");
  ml_quantity_mainpackage_type_label = $(
    "<label class='sub_custom_label_css'>Amount of ML in Main Package:</label>"
  );
  ml_quantity_mainpackage_type_input = $(
    "<input class='form-control-custom' type='number'  step='5.00' min='1'  id='main_ml_quant_input' name='main_ml_quant_input'></input>"
  );
  ...
  var colmd2 = $("<div class='col-md-8'></div>");
  priceperpackage_label = $(
    "<label class='sub_custom_label_css'>Total Purchase Rate:</label>"
  );
  priceperpackage_input = $(
    "<input class='form-control-custom' type='number'  step='50.00' min='0'  id='main_priceperpack_input' name='main_priceperpack_input'></input>"
  );
  ...
  row_four__col_one__package_info_row_div = $(
    "<div class='row' style='padding-bottom:10px'></div>"
  );
  var colmd1 = $("<div class='col-md-4'></div>");
  var colmd2 = $("<div class='col-md-8'></div>");
  quantityofpackage_label = $(
    "<label class='sub_custom_label_css'>Quantity of Main Package :</label>"
  );
  quantityofpackage_input = $(
    "<input class='form-control-custom' type='number' step='1.00' min='0'  id='main_quantity_input' name='main_quantity_input'></input>"
  );
...
function addRowDivFourCustomSyrup() {
  ...
  var calculate_button = $(
    '<button class="calcbtn fa fa-calculator" style="width:inherit" onclick="calculate_medPriceQuantity_Custom()">  Calculate</button>'
  );
  savetomedstrgForm_button = $(
    '<button class="save_btn_normal fa fa-save"  style="width:inherit" onclick="saveMedicineToWhStock_Bottle()">  Save</button>'
  );
...
function saveMedicineToWhStock_Bottle() {
  ...
  DispensoryBottle_unit_quant = parseFloat(
    $("#DispensoryBottle_unit-quant").text()
  );
  DispensoryBottle_total_quant = parseFloat(
    $("#DispensoryBottle_total-quant").text()
  );
  DispensoryBottle_unit_price = parseFloat(
    $("#DispensoryBottle_unit-price").text()
  );
  ...
  if (
    ...
    manufactor_date === "" ||
    exp_date === "" ||
    MainBottle_total_quant === NaN ||
    MainBottle_unit_price === NaN ||
    DispensoryBottle_unit_quant === NaN ||
    DispensoryBottle_unit_quant === NaN ||
    DispensoryBottle_total_quant === NaN ||
    DispensoryBottle_unit_price === NaN
  ) {
    alert("Please Complete the Form");
    return;
  }
  if (
    DispensoryBottle_unit_price === Infinity ||
    MainBottle_unit_price === Infinity
  ) {
    alert("Invalid Entry");
    return;
  }
...
function subPackageCustomHtml(
  pack_type,
  sublevel_custom,
  subpack_customtype_list
) {
  var medstorage_main_col_div = $("#cont_row_div_two");
  var row_div = $(
    "<div class='row subpacks_custom' id='subpack_custom_row_div_sublevel_custom" +
      sublevel_custom +
      "' ></div>"
  );
  var col_one__row_div = $(
    "<div class='col-md-8'  id='subpack_custom_info_col_div_sublevel_custom" +
      sublevel_custom +
      "'></div>"
  );
  var row_one__col_one__row_div = $(
  ...
  var subpack_customtype_label = $(
    "<label class='sub_custom_label_css'>Sub Package Type</label>"
  );
  var subpack_customtype_select = $(
    "<select class='form-control-custom' onchange='subpack_customtypeOnSelect($(this))' id='subpack_customtype_select" +
      pack_type +
      "'></select>"
  );
  var option = $("<option value='--'>--</option>");
  ...
  var quantity_subpack_customtype_label = $(
    "<label class='sub_custom_label_css'>Capicity of Dispensory Bottle ( ML ) : </label>"
  );
  var quantity_subpack_customtype_input = $(
    "<input class='form-control-custom' type='number' step='1.00' min='1' id='sublevel" +
      sublevel_custom +
      "' name='sublevel_input'></input>"
  );
  var delete_btn = $(
    "<button class='fas fa-2x  fa-times-circle' id= 'delBtn' onclick='deleteSubPackCustomdiv($(this))'></button>"
  );
...
function deleteSubPackCustomdiv(element) {
  ...
  console.log(
    $("#subpack_custom_row_div_sublevel_custom1 option:selected").val()
  );
  var package_type = $(
    "#subpack_custom_row_div_sublevel_custom1 option:selected"
  ).val();
...
function calculate_medPriceQuantity_Custom() {
  ...
  $("[id^=subpack_custom_row_div_sublevel]")
    .map(function () {
  ...
  $("#calc_res_info_form").empty();
  var container = $(
    '<div class="col-md-12 container-left" id="container_sublevels"> </div>'
  );
  var row = $("<div class='row' id='row_sublevels'></div>");
  var col = $("<div class='col-md-12' id='row_sublevels_cols'></div>");

  var row_div_one = $(
    "<div class='row' id='sublevelsmainrowquant_" + selectedoption + "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  var row1_col_one__row_div = $("<div class='row'></div>");
  ...
  quanttotal_label = $(
    "<label id='" + selectedoption + "_total' >No of Bottles:</label>"
  );
  quanttotal_input = $(
    "<span id='" +
      selectedoption +
  ...
  var row_div_two = $(
    "<div class='row' id='sublevelsmainrowmlunit_" + selectedoption + "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  var row2_col_one__row_div = $("<div class='row'></div>");
  colmd1 = $("<div class='col-md-9'></div>");
  colmd2 = $("<div class='col-md-2'></div>");
  mlperunit_label = $(
    "<label id='" + selectedoption + "_unit' >ML in 1 Bottle:</label>"
  );
  mlperunit_input = $(
    "<span id='" +
      selectedoption +
      "_unit-ml' class='form-control-custom-static'>" +
      mlinonebottle +
      "</span>"
  );
  ...
  var row_div_three = $(
    "<div class='row' id='sublevelsmainrowpriceunit_" +
      selectedoption +
      "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  ...
  colmd2 = $("<div class='col-md-2'></div>");
  priceofonepack_label = $(
    "<label id='" + selectedoption + "_unit' >Price of one Bottle:</label>"
  );
  priceofonepack_input = $(
    "<span id='" +
      selectedoption +
  ...
  var row_div_four = $(
    "<div class='row' id='sublevelsSubrowquantunit_" +
      sublevel_customtemplist[0][1] +
      "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  ...
  quantperunit_label = $(
    "<label id='" +
      sublevel_customtemplist[0][1] +
      "_unit' >No of Dispensory Bottles in 1 Bottle :</label>"
  );
  quantperunit_input = $(
    "<span id='" +
      sublevel_customtemplist[0][1] +
      "_unit-quant' class='form-control-custom-static'>" +
      noofdespbottleinonebottle +
      "</span>"
  );
  ...
  var row_div_five = $(
    "<div class='row' id='sublevelsSubrowmlunit_" +
      sublevel_customtemplist[0][1] +
      "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  ...
  mlperunit_label = $(
    "<label id='" +
      sublevel_customtemplist[0][1] +
      "_unit' >Ml in one Dispensory Bottles Ml in one Dispensory Bottles:</label>"
  );
  mlperunit_input = $(
    "<span id='" +
      sublevel_customtemplist[0][1] +
  ...
  var row_div_six = $(
    "<div class='row' id='sublevelsSubrowquanttotal_" +
      sublevel_customtemplist[0][1] +
      "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  ...
  quantTotal_label = $(
    "<label id='" +
      sublevel_customtemplist[0][1] +
      "_total' >Total Dispensory Bottles:</label>"
  );
  quantTotal_input = $(
    "<span id='" +
      sublevel_customtemplist[0][1] +
  ...
  var row_div_seven = $(
    "<div class='row' id='sublevelsSubrowpriceunit_" +
      sublevel_customtemplist[0][1] +
      "'></div>"
  );
  var col_one__row_div = $("<div class='col-md-12'></div>");
  ...
  priceperunit_label = $(
    "<label id='" +
      sublevel_customtemplist[0][1] +
      "_unit' >Price of one Dispensory Bottle:</label>"
  );
  priceperunit_input = $(
    "<span id='" +
      sublevel_customtemplist[0][1] +
      "_unit-price' class='form-control-custom-static'>" +
      priceofonedespbottle +
      "</span>"
  );
...
function addMedicineToDespStockForm() {
  ...
  $("#container-med-dashboard").append(
    "<h2 class='center_h_tag_forms'>Add Medicine To Despensory</h2>"
  );
  $("#container-med-dashboard").append("<hr class='custom_hr'>");
  var main_row_div = $("<div class='row removerowmargins_div'></div>");
  $("#container-med-dashboard").append(main_row_div);
  var main_col_div_1 = $(
    "<div class='col-md-8 genformdiv1'id='despstorage_first_col_div'></div>"
  );
  var main_col_div_2 = $(
    "<div class='col-md-4 '  id='despstorage_second_col_div'></div>"
  );
...
function despMedFormMainColOne(main_col_div_1) {
  ...
  med_name_label = $(
    "<label for='medicine_name_tag' class='custom_label_css'>Medicine</label>"
  );
  colmd1.append(med_name_label);
  med_name_input = $(
    "<input class='form-control-custom' id='medicine_name_tag' onfocusout='focusOut_medicineNameDesp($(this))'  class='custom_input_css'>"
  );
  colmd2.append(med_name_input);
  row__col_one__row_div_one.append(colmd1);
  row__col_one__row_div_one.append(colmd2);
  ...
  colmd2 = $("<div class='col-md-6'></div>");
  batchno_label = $(
    "<label class='custom_label_css float-right'>Batch Number </label>"
  );
  colmd1.append(batchno_label);
  batchno_input = $(
    "<input class='form-control-custom' type='number'  id='batchno_input'  class='custom_input_css'></input>"
  );
  colmd2.append(batchno_input);
  row__col_three__row_div_one.append(colmd1);
  row__col_three__row_div_one.append(colmd2);
  ...
  var add_button = $(
    "<button class='fa fa-plus-circle add_all_stk' onclick='addAllstck()' >  Add All Stock</button>"
  );
...
function despMedFormMainColTwo(main_col_div_2) {
  var row_div_one = $(
    "<div class='row despstorage_second_col_div_three_rows genformdiv2' id='med_in_stock_table_row'></div>"
  );
  var col_one__row_div_one = $("<div class='col-md-12'></div>");
  var table = $(
    '<table id="med_in_stock" class="datatablecss_med"  width="100%"></table>'
  );

  col_one__row_div_one.append(table);
  row_div_one.append(col_one__row_div_one);
  ...
  if (medicine_batch_in_stock_dict[medicine_name] !== undefined) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        row1__col_one__row_div_two = $(
          "<div class='row desp_col_div_rows'></div>"
        );
        var colmd1 = $("<div class='col-md-2'></div>");
        var colmd2 = $("<div class='col-md-2'></div>");
        boxamount_label = $(
          "<label class='custom_label_css'>Number of Boxes:</label>"
        );
        colmd1.append(boxamount_label);
        boxamount_input = $(
          "<input class='form-control-custom' type='number' id='noofboxes_input' class='custom_input_css' value='0'> </input>"
        );
        colmd2.append(boxamount_input);
        row1__col_one__row_div_two.append(colmd1);
        row1__col_one__row_div_two.append(colmd2);
        if (strip_unit !== "-") {
          row3__col_one__row_div_two = $(
            "<div class='row desp_col_div_rows'></div>"
          );
          var colmd1 = $("<div class='col-md-2'></div>");
          var colmd2 = $("<div class='col-md-2'></div>");
          pieceamount_label = $(
            "<label class='custom_label_css'>No of Strips:</label>"
          );
          colmd1.append(pieceamount_label);
          pieceamount_input = $(
            "<input class='form-control-custom' type='number' id='noofstrips_input' value='0'  class='custom_input_css'> </input>"
          );
        ...
        row2__col_one__row_div_two = $(
          "<div class='row desp_col_div_rows'></div>"
        );
        var colmd1 = $("<div class='col-md-2'></div>");
        var colmd2 = $("<div class='col-md-2'></div>");
        pieceamount_label = $(
          "<label class='custom_label_css'>No of Pieces:</label>"
        );
        colmd1.append(pieceamount_label);
        pieceamount_input = $(
          "<input class='form-control-custom' type='number' id='noofpieces_input' value='0'  class='custom_input_css'> </input>"
        );
        colmd2.append(pieceamount_input);
        ...
        var row_div_three = $(
          "<div class='row desp_col_div_rows' id='row_div_three'></div>"
        );
        col_one__row_div_three = $("<div class='col-md-12'></div>");
        row__col_one__row_div_three = $("<div class='row'></div>");
        var colmd1 = $("<div class='col-md-12'></div>");
        var save_button = $(
          "<button class='save_btn' onclick='SaveToDespStock()' style='width:inherit;'>Save</button>"
        );
        ...
        var active_row_div_two = $(
          "<div class='row despstorage_second_col_div_three_rows genformdiv2' id='active_batch_data_row'></div>"
        );
        var col_one__row_div_two = $("<div class='col-md-12'></div>");
        var row__col_one__row_div_two = $(
          "<div style=' font-size: 0.8rem;font-weight: bold;text-align: center;padding-bottom: 20px;'>Active Batch Stock Data</div>"
        );
        ...
        boxamount_label = $(
          "<label class='custom_label_css'>No of Boxes Stored:</label>"
        );
        colmd1.append(boxamount_label);
        var colmd2 = $("<div class='col-md-6'></div>");
        boxamount_input = $(
          "<input class='form-control-custom' type='number' id='active_box_input' class='custom_input_css' value='" +
            boxes +
            "' disabled> </input>"
        );
        colmd2.append(boxamount_input);
        ...
        if (strip_unit !== "-") {
          var colmd1 = $("<div class='col-md-6'></div>");
          pieceamount_label = $(
            "<label class='custom_label_css'>No of Strips Stored:</label>"
          );
          colmd1.append(pieceamount_label);
          var colmd2 = $("<div class='col-md-6'></div>");
          pieceamount_input = $(
            "<input class='form-control-custom' type='number' id='active_strip_input' value='" +
              strips +
              "'  class='custom_input_css' disabled> </input>"
          );
          colmd2.append(pieceamount_input);
          row3__col_one__row_div_two.append(colmd1);
          row3__col_one__row_div_two.append(colmd2);
        ...
        var colmd1 = $("<div class='col-md-6'></div>");
        pieceamount_label = $(
          "<label class='custom_label_css'>No of Pieces Stored:</label>"
        );
        colmd1.append(pieceamount_label);
        var colmd2 = $("<div class='col-md-6'></div>");
        pieceamount_input = $(
          "<input class='form-control-custom' type='number' id='active_piece_input' value='" +
            pieces +
            "'  class='custom_input_css' disabled> </input>"
        );
        colmd2.append(pieceamount_input);
        ...
        var temp_row_div_three = $(
          "<div class='row despstorage_second_col_div_three_rows genformdiv2' id='med_in_temp_stock_datatable_row'></div>"
        );
        var col_one__row_div_three = $("<div class='col-md-12'></div>");

        var row2__col_one__row_div_three = $("<div></div>");
        table = $(
          '<table id="med_in_temp_stock" class="datatablecss_med" width="100%" ></table>'
        );
        ...
        $(function () {
          $("#med_in_temp_stock").append(
            '<caption class="custom_label_css"  style="font-size:0.9rem;font-weight: bold;text-align: center;caption-side: top;">InActive Batches Of Medicine </caption>'
          );
...
function updateMedicineForm() {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#main_page_content").empty();
      var main_page_content = $("#main_page_content").append(
        '<div class="container-fluid" id="container-update-medinfo"></div>'
      );

      $("#container-update-medinfo").append(
        "<h2 class='center_h_tag_forms'>Update Medicine To Inventory</h2>"
      );
      $("#container-update-medinfo").append("<hr>");
      var main_row_div1 = $(
        "<div class='row removerowmargins_div' id='update_med_info_mr1'></div>"
      );
      var main_row_div2 = $(
        "<div class='row removerowmargins_div' id='update_med_info_mr2'></div>"
      );
      ...
      var main_col_div1 = $(
        "<div class='col-md-12 genformdiv2'  id='update_med_info_mc1' ></div>"
      );
      var table = $(
        '<table id="update_med_table" width="100%"  class="datatablecss_med" ></table>'
      );
      $(main_col_div1).append(table);
      $(main_row_div1).append(main_col_div1);
      var main_col_div2 = $(
        "<div class='col-md-12 genformdiv1' id='update_med_info_mc2' ></div>"
      );
...
function createMedicineUpdateForm(medid) {
  ...
  var package_info_row_div = $("<div class='row'></div>");
  var row_code_name = $("<div class='row' id='update_code_name_row'></div>");
  var row_div_three = $("<div class='row'></div>");

  var row_div_four = $("<div class='row'></div>");
  var weight_litre_div_row = $(
    "<div class='row' id='update_weight_litre_div_row'></div>"
  );
  ...
  medname = medicine_name_dict_update[medid]["name"].toUpperCase();

  col_two__row_div_one__col_one__row_div_one = $(
    "<div class='col-md-8 ' style=' padding-top:10px' id='update_med_name_input_div'><input value='" +
      medname +
      "' type='text' id='update_med_name_input' class='form-control-custom'></input></div>"
  );
  ...
  col_two__row_div_one__col_two__row_div_one = $(
    "<div class='col-md-8 '><input value='" +
      medicine_name_dict_update[medid]["type"] +
      "' type='text' id='update_med_type_input' class='form-control-custom' disabled></input></div>"
  );
  $(row_div_one__col_two__row_div_one).append(
    col_one__row_div_one__col_two__row_div_one
  );
  ...
  //  Html of row 3 starting
  update_code_name1 = $(
    "<div class='col-md-4' style=' padding-top:10px'><label class='custom_label_css'>Code Name</label></div>"
  );

  update_code_name2 = $(
    "<div class='col-md-8 ' style=' padding-top:10px' id='update_med_name_input_div'><input value='" +
      medicine_name_dict_update[medid]["code_name"] +
      "' type='text' id='update_med_codename_input' class='form-control-custom'></input></div>"
  );
  row_code_name.append(update_code_name1);
  row_code_name.append(update_code_name2);
  ...
  p__col_two__package_info_row_div = $(
    "<input type='textarea' value='" +
      medicine_name_dict_update[medid]["med_det"] +
      "' class='form-control-custom' id='update_med_details'></input>"
  );
  col_two__package_info_row_div.append(p__col_two__package_info_row_div);
  package_info_row_div.append(col_one__package_info_row_div);
  package_info_row_div.append(col_two__package_info_row_div);
  ...
  if (medicine_name_dict_update[medid]["type"] === "Tablet") {
    ...
    p__col_two__weight_div_row = $(
      "<input class='form-control-custom weight_litre_class' value='" +
        medicine_name_dict_update[medid]["weight"] +
        "' type='number' min=0 id='update_med_weight_input'></input>"
    );
    ...
    p__col_two = $(
      "<input class='form-control-custom weight_litre_class' value='" +
        medicine_name_dict_update[medid]["litre"] +
        "' type='number' min=0 id='update_med_litre_input'></input>"
    );
  ...
  save_btn_label = $(
    "<button class='save_btn fa fa-save' style='width:inherit' onclick='UpdateMedicineToDb()'>  Update</button>"
  );
  col_two__row_div_four.append(save_btn_label);
  ...
  $(main_col_div2).append(row_code_name);
    ...
    $("#dialog-confirm").dialog({
      ...
      buttons: {
        Update: function () {
          ...
          code_name = $("#update_med_codename_input").val();
          ...
          sendAjaxReqToUpdateMedicineToDb(
            medid,
            medicine_name,
            type,
            med_details,
...
function sendAjaxReqToUpdateMedicineToDb(
  medid,
  medicine_name,
  type,
  med_details,
...
function generateMedBarCodes() {
  $.ajax({
    type: "GET",
    dataType: "json",
    data: {},
    url: "update_medicine_batches_barcode",
    success: function (data) {
      medicine_batch_barcode_list = data["medicine_batch_barcode_list"];
      medicine_batch_dict_update = JSON.parse(
        data["medicine_batch_dict_update"]
      );

      $("#main_page_content").empty();
      var main_page_content = $("#main_page_content").append(
        '<div class="container-fluid" id="container_med_batch_barcode"></div>'
      );

      $("#container_med_batch_barcode").append(
        "<h2 class='center_h_tag_forms'>Generate Medicine BarCodes</h2>"
      );
      $("#container_med_batch_barcode").append("<hr>");
      var main_row_div1 = $(
        "<div class='row removerowmargins_div' id='medicine_batch_table'></div>"
      );
      var main_row_div2 = $(
        "<div class='row removerowmargins_div' id='medicine_batch_edit_field'></div>"
      );

      $("#container_med_batch_barcode").append(main_row_div1);
      $("#container_med_batch_barcode").append(main_row_div2);

      var main_col_div1 = $(
        "<div class='col-md-12 genformdiv2'  id='update_med_batch_field1' ></div>"
      );
      var table = $(
        '<table id="medicine_batches_barcode" width="100%"  class="datatablecss_med" ></table>'
      );
      $(main_col_div1).append(table);
      $(main_row_div1).append(main_col_div1);
      var main_col_div2 = $(
        "<div class='col-md-12 genformdiv1' id='med_batch_field' ></div>"
      );

      $(main_row_div2).append(main_col_div2);

      medicineBatchDataTable(medicine_batch_barcode_list);
    },
  });
}
function medicineBatchDataTable(medicine_batch_list) {
  $("#medicine_batches_barcode").empty();
  $(function () {
    $("#medicine_batches_barcode").append(
      '<caption style="caption-side: top;text-align: center;" class="datatable_heading_label">Medicine Batches</caption>'
    );
    med_batch_table_datatable = $("#medicine_batches_barcode").DataTable({
      data: medicine_batch_list,
      columns: [
        { title: "Id" },
        { title: "Medicine Name" },
        { title: "Code Name" },
        { title: "BarCode" },
        { title: "Storage Stock ID" },
        { title: "Batch ID" },
        { title: "Creation Date" },
      ],
      paging: false,
    ...
    $("#medicine_batches_barcode tbody").on("click", "tr", function () {
      if ($(this).hasClass("selected")) {
      } else {
        medbatchId = $(this).find("td").eq(0).text();
        med_batch_table_datatable.$("tr.selected").removeClass("selected");
        $(this).addClass("selected");

        createMedicineBatchForm(medbatchId);
        // $("html,body").animate(
        //   {
        //     scrollTop: $("#med_batch_field").offset().top,
        //   },
        //   "slow"
        // );
      }
...
function createMedicineBatchForm(medbatchId) {
  $("#med_batch_field").empty();
  var medBatchRow = $(
    "<div class='row medBatchId' id='" +
      medbatchId +
      "' style='padding-bottom:10px;'></div>"
  );
  var updateMedBatchBtnRow = $(
    "<div class='row' id='update_med_batch_btn_row'></div>"
  );

  medFieldCol = $("<div class='col-md-6'></div>");
  $(medBatchRow).append(medFieldCol);

  medFieldRow = $("<div class='row' ></div>");

  $(medFieldCol).append(medFieldRow);

  col_one = $(
    "<div class='col-md-4' style=' padding-top:10px'><label class='custom_label_css'>BarCode</label></div>"
  );
  barcode = medicine_batch_dict_update[medbatchId]["barcode"];

  col_two = $(
    "<div class='col-md-8 ' style=' padding-top:10px' id='update_med_batch_barcode_div'><input value='" +
      barcode +
      "' type='text' id='update_med_batch_barcode' class='form-control-custom'></input></div>"
  );

  $(medFieldRow).append(col_one);
  $(medFieldRow).append(col_two);
  ...
  save_btn_label = $(
    "<button class='save_btn fa fa-save' style='width:inherit' onclick='UpdateBarCode()'>  Update</button>"
  );
  col2.append(save_btn_label);

  updateMedBatchBtnRow.append(col1);
  updateMedBatchBtnRow.append(col2);
  updateMedBatchBtnRow.append(col3);

  main_col_div = $("#med_batch_field");
  $(main_col_div).append(medBatchRow);
  $(main_col_div).append(updateMedBatchBtnRow);
}
function UpdateBarCode() {
  $(function () {
    ...
    $("#dialog-confirm").dialog({
      ...
      buttons: {
        Update: function () {
          barcode = $("#update_med_batch_barcode").val();

          medBatchId = $(".medBatchId").attr("id");

          sendAjaxReqToUpdateMedicineBatchBarCodeToDb(medBatchId, barcode);
...
function sendAjaxReqToUpdateMedicineBatchBarCodeToDb(medBatchId, barcode) {
  $(".modal-loading").show();
  ...
  $.ajax({
    ...
    url: "/update_medicine_batches_barcode",
    data: {
      medBatchId: JSON.stringify(medBatchId),
    ...
    success: function (data) {
      ...
      med_batch_table_datatable.clear().draw();
      for (var i in medicine_batch_barcode_list) {
        med_batch_table_datatable.row
          .add(medicine_batch_barcode_list[i])
          .draw();
      }
    },
  });
